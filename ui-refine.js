...
(()=>{
"use strict";
if(!document.querySelector('link[data-pm-desktop-v1]')){
  const desktopCss=document.createElement("link");
  desktopCss.rel="stylesheet";
  desktopCss.href="./desktop-v1.css";
  desktopCss.dataset.pmDesktopV1="true";
  document.head.appendChild(desktopCss);
}

const byId=id=>document.getElementById(id);
const LANG_KEY="pm-language";
let currentLang=["en","es","sr"].includes(localStorage.getItem(LANG_KEY))?localStorage.getItem(LANG_KEY):"en";
...
const ICONS={
 ...
 more:`<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="5" cy="12" r="1.6"></circle><circle cx="12" cy="12" r="1.6"></circle><circle cx="19" cy="12" r="1.6"></circle></svg>`
};
...
const PM_RELEASE={
  version:"1.6.6",
  highlights:["Desktop Layout","Wider Explore","Roomier Library"]
};
...
byId("list")?.addEventListener("click",async e=>{const more=e.target.closest("[data-pm-more]");if(more){e.preventDefault();e.stopImmediatePropagation();const menu=more.nextElementSibling,opening=!!menu?.hidden;document.querySelectorAll(".pm-card-menu:not([hidden])").forEach(x=>{if(x!==menu)x.hidden=true});if(menu)menu.hidden=!opening;more.setAttribute("aria-expanded",String(opening));return}if(!e.target.closest(".pm-card-menu"))document.querySelectorAll(".pm-card-menu:not([hidden])").forEach(x=>x.hidden=true);const card=e.target.closest(".card");if(!card)return;const p=promptById(card.dataset.use);if(!p)return;if(selectMode){e.preventDefault();e.stopImmediatePropagation();selectedIds.has(p.id)?selectedIds.delete(p.id):selectedIds.add(p.id);render();return}const copy=e.target.closest("[data-pm-copy]"),share=e.target.closest("[data-pm-share]"),edit=e.target.closest("[data-pm-edit]"),trash=e.target.closest("[data-pm-delete]");if(copy||share||edit||trash){e.preventDefault();e.stopImmediatePropagation();if(copy)return copyPromptFromCard(p,copy);if(share)return sharePrompt(p,share);if(edit)return openEdit(p);if(trash)return deleteOne(p)}e.preventDefault();e.stopImmediatePropagation();openUse(p)},true);
...
byId("diagnosticsCopy")?.addEventListener("click",()=>setTimeout(()=>flashButton(byId("diagnosticsCopy"),t("copied")),0),true);byId("export")?.addEventListener("click",()=>setTimeout(()=>flashButton(byId("export"),`✓ ${currentLang==="es"?"Exportado":currentLang==="sr"?"Izvezeno":"Exported"}`),0),true);document.addEventListener("click",e=>{const b=e.target.closest("#saveDisplayName");if(!b)return;setTimeout(()=>flashButton(b,`✓ ${t("saved")}`),0)},true);
...
byId("settingsLogoutBtn")?.addEventListener("click",()=>{
