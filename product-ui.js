...
function fromCSV(t){let a=csv(t);if(!a.length)return[];let known=["prompt","content","text","title","name","category","platform","model","models","source","url"],hdr=a[0].some(x=>known.includes(k(x)));heads=hdr?a[0].map((x,i)=>n(x)||`Column ${i+1}`):a[0].map((_,i)=>`Column ${i+1}`);return(hdr?a.slice(1):a).filter(r=>r.some(n)).map(r=>Object.fromEntries(heads.map((h,i)=>[h,r[i]??""])))}
...
function automap(){let A={prompt:["prompt","content","text","instruction","instructions"],title:["title","name"],category:["category","type","topic"],platform:["platform","platforms","tool"],model:["model","models"],source:["sourceurl","source","url","link"]};map={};for(let [f,as]of Object.entries(A)){let h=heads.find(x=>as.includes(k(x)));if(h)map[f]=h}if(!map.prompt&&heads.length==1)map.prompt=heads[0]}
...
function normalized(){let old=new Set;try{for(let p of prompts||[])old.add(n(p.content).toLowerCase().replace(/\s+/g," "))}catch{}let seen=new Set;return rows.map((o,i)=>{let content=val(o,"prompt"),sig=content.toLowerCase().replace(/\s+/g," "),dup=!!content&&(old.has(sig)||seen.has(sig));if(content)seen.add(sig);return{valid:!!content,dup,title:val(o,"title")||title(content,i),content,category:val(o,"category")||cat(content),platform:platform(val(o,"platform")),models:(window.pmModelSupport?.parse(val(o,"model"))||[]),source:val(o,"source")}})}
function mapping(){let F=[["prompt","Prompt text *"],["title","Title"],["category","Category"],["platform","Platform"],["model","Model(s)"],["source","Source URL"]];q("importMapping").innerHTML=F.map(([f,l])=>`<label class="pm-import-field"><span>${l}</span><select data-map="${f}"><option value="">${f=="prompt"?"Select column":"Auto / none"}</option>${heads.map(h=>`<option value="${eh(h)}"${map[f]==h?" selected":""}>${eh(h)}</option>`).join("")}</select></label>`).join("")}
...
if(typeof filtered==="function"&&!filtered.pm175){
 ...
 const enhanced=function(){
   ...
   return (prompts||[]).filter(p=>{
     ...
     const modelOK=window.pmModelSupport?.matches?window.pmModelSupport.matches(p):true;
     ...
     return originOK&&catOK&&platformOK&&modelOK&&searchOK;
...
if(typeof saveCatalog==="function"&&!saveCatalog.pm179){
  ...
  const enhancedSaveCatalog=async function(x){
    ...
    const item={
      ...
      createdAt:Date.now(),
      platforms:["general"],
      models:Array.isArray(x.models)?x.models:(x.model?[String(x.model)]:[]),
      useCount:0,
      lastUsedAt:null,
...
/* ===== M1.8 ===== */
(()=>{"use strict";
/* Release-facing UI only: keep What's New current and ensure model metadata is
   visible on Library cards, not only inside the Use Prompt dialog. */
const COPY={
  en:{title:"A much bigger prompt catalog.",copy:"Explore now grows from multiple curated prompt sources, with quality filtering and deduplication before prompts reach the catalog.",highlights:["750+ quality target","Multiple curated sources","Model labels in Library"]},
  es:{title:"Un catálogo de prompts mucho mayor.",copy:"Explorar crece ahora a partir de varias fuentes de prompts seleccionadas, con filtros de calidad y deduplicación antes de llegar al catálogo.",highlights:["Objetivo: 750+ prompts de calidad","Varias fuentes seleccionadas","Modelos visibles en Biblioteca"]},
  sr:{title:"Mnogo veći katalog promptova.",copy:"Istraži sada raste iz više odabranih izvora promptova, uz proveru kvaliteta i uklanjanje duplikata pre ulaska u katalog.",highlights:["Cilj: 750+ kvalitetnih promptova","Više odabranih izvora","Modeli vidljivi u Biblioteci"]}
};
function lang(){const x=localStorage.getItem("pm-language")||document.documentElement.lang||"en";return COPY[x]?x:"en"}
function updateWhatsNew(){
  const card=document.querySelector(".pm-whats-new");if(!card)return;
  const c=COPY[lang()];
  const version=card.querySelector(".pm-version");if(version&&version.textContent!=="V1.8")version.textContent="V1.8";
  const title=card.querySelector("[data-pm-whats-title]");if(title&&title.textContent!==c.title)title.textContent=c.title;
  const copy=card.querySelector("[data-pm-whats-copy]");if(copy&&copy.textContent!==c.copy)copy.textContent=c.copy;
  const list=card.querySelector("ul");const html=c.highlights.map(x=>`<li>${x}</li>`).join("");if(list&&list.innerHTML!==html)list.innerHTML=html;
}
function modelLabel(value){return String(value||"").toLowerCase()==="multimodel"?"Multimodel":String(value||"").trim()}
...
function decorateLibraryModels(){
  document.querySelectorAll("#list .card[data-use]").forEach(card=>{
...
function sync(){updateWhatsNew();decorateLibraryModels()}
const home=document.getElementById("homeView"),list=document.getElementById("list");
if(home)new MutationObserver(updateWhatsNew).observe(home,{childList:true,subtree:true});
if(list)new MutationObserver(decorateLibraryModels).observe(list,{childList:true,subtree:true});
new MutationObserver(updateWhatsNew).observe(document.documentElement,{attributes:true,attributeFilter:["lang"]});
queueMicrotask(sync);
})();
