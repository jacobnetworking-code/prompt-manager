...
function catalogSaved(x){return prompts.some(p=>p.acquisitionType==="catalog"&&String(p.externalId)===String(x.id)&&p.sourceName==="prompts.chat")}
...
async function saveCatalog(x){if(!x||catalogSaved(x))return;let item={title:x.title,content:x.prompt,source:"https://prompts.chat/",categoryId:categories.some(c=>c.id===slug(x.category))?slug(x.category):"general",createdAt:Date.now(),platforms:["general"],useCount:0,lastUsedAt:null,acquisitionType:"catalog",sourceName:"prompts.chat",externalId:String(x.id)};await add("prompts",item);await refresh();renderExplore();toast("Saved to Library")}
