...
const ICON_EDIT=`<svg viewBox="0 0 24 24"><path d="M4 20h4l11-11a2.8 2.8 0 0 0-4-4L4 16v4Z"/><path d="m13.5 6.5 4 4"/></svg>`;
const ICON_DUP=`<svg viewBox="0 0 24 24"><rect x="8" y="8" width="11" height="11" rx="2"/><path d="M16 8V6a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h2"/></svg>`;
const ICON_DELETE=`<svg viewBox="0 0 24 24"><path d="M4 7h16"/><path d="M9 7V4h6v3"/><path d="M7 7l1 13h8l1-13"/></svg>`;
...
async function openDetail(id){const d=detailDialog();d.dataset.chainId=id;d.innerHTML=`<div class="pm-chain-detail-loading">Loading chain…</div>`;lockBackground();d.showModal();try{const c=await getChain(id),steps=c.steps||[];d.innerHTML=`<div class="pm-chain-detail-sheet">
 ...
 <div class="pm-chain-title-row"><h2>${esc(c.title)}</h2><div class="pm-chain-detail-actions"><button data-chain-share-top="${esc(id)}" aria-label="Share">${ICON_SHARE}</button><button data-detail-menu="${esc(id)}" aria-label="More actions">•••</button><button data-detail-close aria-label="Close">×</button></div></div>
 ...
 <div class="menu pm-chain-detail-menu pm-library-menu" hidden><button data-chain-edit="${esc(id)}">${ICON_EDIT}<span>Edit chain</span></button><button data-chain-duplicate="${esc(id)}">${ICON_DUP}<span>Duplicate chain</span></button><button class="danger" data-chain-delete-v2="${esc(id)}">${ICON_DELETE}<span>Delete chain</span></button></div>
